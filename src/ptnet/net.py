...
#            if (e.m) : s += ' fillcolor=blue'
    ...
    def read (self, f, fmt='pnml') :
        ...
        if fmt == 'll_net' : return self.__read_llnet (f)
    ...
    def __read_pep (self, f) :
        raise Exception, 'reading LL_NET files is not yet implemented'
...
def load_net (path, fmt="pnml", prefix="ptnet: ") :
    net = Net ()
    try :
        size = size_to_human (os.path.getsize (path))
        print "%sreading net file '%s', %s, %s format" % \
                (prefix, path, size, fmt.upper ())
        f = open (path, 'r')
        net.read (f, fmt=fmt)
        f.close ()
    except Exception as e :
        raise Exception, "'%s': %s" % (path, e)
    print "%sdone, %d transitions, %d places" % \
            (prefix, len (net.trans), len (net.places))
    print "%sfirst 3 transitions are:" % prefix
    for t in net.trans[:3] :
        print "%s%s" % (prefix, str (t))
    return net

def save_net (path, net, fmt="pnml", prefix="ptnet: ") :
    try :
        print "%swriting file '%s', %d transitions, %d places, %s format" \
                % (prefix, path, len (net.trans), len (net.places), fmt.upper())
        f = open (path, 'w')
        net.write (f, fmt)
        f.close ()
        size = size_to_human (os.path.getsize (path))
